...
function AtlasFrameHelper.buildSparrowQuad(animationList, texture)
	if texture == nil then
		assert("Attempt to build an SparrowAtlas Quad without a texture, did you forget to set a texture to an AnimatedSprite?")
		return
	end
	local frames = {}
	for i=1, #animationList do
		...
		if trimmed then
			local frameMargin = {x = cfg.frameX, y = cfg.frameY, width = cfg.frameWidth - frameRect.width, height = cfg.frameHeight - frameRect.height}
		...
		table.insert(frames, {
			quad = love.graphics.newQuad(frameRect.x, frameRect.y, frameRect.width, frameRect.height, texture:getDimensions()),
